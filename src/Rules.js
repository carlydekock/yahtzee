...
class FullHouse extends Rule {
  evalRoll = dice => {
    let result = this.freq(dice); //this should be [2,3] or [3,2]
    return (result.includes(2) && result.includes(3)) ? this.score : 0;
  }
}

/** Check for small straights. 4 numbers total*/

class SmallStraight extends Rule {
  evalRoll = dice => {
    const d = new Set(dice);
    //straight can be either 234 + either 1 or 5
    if(d.has(2) && d.has(3) && d.has(4) && (d.has(1) || d.has(5))){
      return this.score;
    }

    //or straight can be 345 + either 2 or 6
